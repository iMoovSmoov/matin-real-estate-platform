        ...
        return (
          <div key={f.q} className="bg-ink-900/60">
            <button
              ...
              className="flex min-h-[44px] w-full cursor-pointer items-center justify-between gap-4 px-6 py-5 text-left font-medium text-white transition-colors hover:bg-white/[0.04] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-emerald-400/60"
              aria-expanded={isOpen}
              aria-controls={panelId}
            >
              <span>{f.q}</span>
              <ChevronDown
                className={`h-4 w-4 shrink-0 text-emerald-400 transition-transform duration-200 motion-reduce:transition-none ${isOpen ? "rotate-180" : ""}`}
              />
            </button>
