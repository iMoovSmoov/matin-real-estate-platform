...
export default function AiStudioPage() {
  return (
    <div className="mx-auto max-w-[1400px] space-y-6 px-4 py-6 md:px-6 md:py-8">
      {/* Hero */}
      <div className="relative overflow-hidden rounded-2xl border border-ink/[0.08] bg-gradient-to-br from-ink/[0.04] to-ink/[0.02] px-6 py-7">
        <div className="pointer-events-none absolute -right-10 top-0 h-56 w-56 rounded-full bg-ink/[0.08] blur-3xl" />
        <div className="relative max-w-2xl">
          <div className="mb-2 flex items-center gap-2">
            <LiveDot tone="success" />
            <SectionLabel>AI Studio</SectionLabel>
          </div>
          <h1 className="font-display text-3xl text-ink md:text-[2.3rem]">
          ...
          <div className="mt-5 flex flex-wrap gap-2.5">
            {TRUST.map((t) => {
              ...
              return (
                <div
                  key={t.label}
                  className="inline-flex items-center gap-2 rounded-xl border border-ink/[0.08] bg-white px-3 py-2"
                >
                  <Icon className="h-4 w-4 text-ink" />
                  <span className="text-[0.8rem] font-semibold text-ink">{t.label}</span>
                  <span className="text-[0.74rem] text-slate/65">· {t.note}</span>
                </div>
              );
            })}
          </div>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-ink/[0.06] text-ink ring-1 ring-inset ring-ink/[0.06] transition-colors group-hover:bg-ink/[0.08]">
              ...
              <h3 className="mt-1 font-display text-xl text-ink">{t.name}</h3>
      ...
      <Panel className="px-5 py-4">
        <p className="text-[0.8rem] leading-relaxed text-slate/80">
          <span className="font-semibold text-ink">Grounded in Matin.</span> Every tool is trained on the
