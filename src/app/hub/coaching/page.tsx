...
type CoachingTab = "train" | "scorecard" | "leaderboard";
  ...
  return (
    <div className="mx-auto max-w-[1400px] space-y-5 px-4 py-6 md:px-6 md:py-8">
      ...
      <div className="flex gap-1 border-b border-ink/[0.08]">
        {(["train", "scorecard", "leaderboard"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 py-2.5 text-[0.88rem] font-medium transition-colors capitalize ${
              tab === t
                ? "text-ink border-b-2 border-ink -mb-px"
                : "text-slate/60 hover:text-ink"
            }`}
          >
            {t === "train" ? "Train" : t === "scorecard" ? "Scorecard" : "Leaderboard"}
          </button>
        ))}
      </div>
